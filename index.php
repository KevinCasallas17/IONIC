...
$app->post('/insertar', function ($request, $response, $args) {  //Defino los servicios  $app->post('/updateVeces', function ($request, $response)
	try{
		$json = $request->getBody();
		$data = json_decode($json, true);
		$db =  getDB(); //Carga los datos
...
$app->post('/updateDatos', function ($request, $response, $args) {  //Defino los servicios  $app->post('/updateVeces', function ($request, $response)
	try{
		...
		$sth = $db->prepare("UPDATE datos
							...
							WHERE dataid = ?");//Insertamos información
		$sth->execute(array($data['datNombre'],$data['datApellido'],$data['datEdad'],$data['datDeporte'],$data['datImagen'],$data['dataid'])); //Sustituimos y ejecutamos la consulta
		$response->getBody()->write('{"error":"ok"}'); //Cuando la conexión halla terminado
